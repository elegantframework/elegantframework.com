...
interface Props {
    ...
    pastVersions: Link[];
    /**
     * An upcoming version to be displayed in the dropdown.
     */
    next_version?: Link;
    /**
    ...
    <Menu as="div" className={clsx("relative z-50", className)}>
      ...
      <Menu.Items className="absolute top-full mt-1 py-2 w-40 rounded-lg bg-white shadow ring-1 ring-slate-900/5 text-sm leading-6 font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:highlight-white/5">
        {next_version && (
         <Menu.Item>
            <a
              href={next_version.href}
              className={clsx(
                'block px-3 py-1 hover:bg-slate-50 text-slate-900 dark:bg-slate-600/30 dark:text-white'
              )}
            >
              v{next_version.label}
            </a>
          </Menu.Item> 
        )}
        <Menu.Item disabled>
