---
title: 'How to Style Text'
status: 'published'
author:
  name: 'Brandon Owens'
  picture: 'https://avatars.githubusercontent.com/u/10189130?v=4'
slug: 'how-to-style-text'
description: 'Enhance your Elegant content by adding Styles and Formatting to your documents. This guide will show you how to customize your content and website with different typography settings and formatting.'
coverImage: ''
publishedAt: '2024-02-04T22:35:11.000Z'
---

Adding **Styling** and **Formatting** to your documents allows you to further customize the design and feel of your Elegant website, and take your content to the next level.

This guide will show you how to customize your content and website with different typography settings and formatting.

## Styling Text

A floating menu with styling options will appear whenever you select a text block in the content editor.

Below is a list of included text styling options that you can apply to your content by simply selecting a block of text within the content editor, and then clicking the corresponding styling option within the floating editor menu:

## Bold

Add bold text to highlight certain words or phrases within your documents.



<div class="my-8 shadow-xl"><img class="rounded-xl" src="/api/images/screenshot-2024-02-03-202958-c0Nz.png" alt="null"></div>



## Italic

Add italicized styling to your text to convey a particular emphasis or tone in writing.



<div class="my-8 shadow-xl"><img class="rounded-xl" src="/api/images/screenshot-2024-02-03-221824-A4OD.png" alt="null"></div>



**Note:** It's important to note that excessive use of italicized text can make reading more difficult, so it's generally best to use it sparingly and with a clear purpose.



## Underline

Underlining specific words or phrases can draw attention and highlight crucial information, making it stand out for readers.



<div class="my-8 shadow-xl"><img class="rounded-xl" src="/api/images/screenshot-2024-02-03-221626-g0OD.png" alt="null"></div>



## Strikethrough

Strikethroughs can be used to indicate a change in the author's opinion or stance on a particular topic.



<div class="my-8 shadow-xl"><img class="rounded-xl" src="/api/images/screenshot-2024-02-03-221542-A1OD.png" alt="null"></div>



## Font Size

To set the font size for your text, select a block of text and click on the desired font level from the floating dropdown menu.



<div class="my-8 shadow-xl"><img class="rounded-xl" src="/api/images/screenshot-2024-02-04-111410-czMT.png" alt="null"></div>



## Font Type

To set the font type for your text, select a block of text and click on the desired font type from the floating dropdown menu.



<div class="my-8 shadow-xl"><img class="rounded-xl" src="/api/images/screenshot-2024-02-04-111440-g5ND.png" alt="null"></div>



## Text Color

To set the text color, select a block of text and click on the **Text color** option, and drag the selector to your desired color in the the color panel.



<div class="my-8 shadow-xl"><img class="rounded-xl" src="/api/images/screenshot-2024-02-04-111600-U1Mz.png" alt="null"></div>



## Text Alignment

Set your content to **Justified**, **Align Left,** **Align Center**, or **Align Right** by selecting the desired block of text and clicking the Text Alignment menu from the dropdown menu.



<div class="my-8 shadow-xl"><img class="rounded-xl" src="/api/images/screenshot-2024-02-04-111651-g0OT.png" alt="null"></div>



